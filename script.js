...
import { initDepthField, initDepthThread, bindHeroBoost } from "./depth.js";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initDepthThread(); // The single connecting path strung through the whole scroll journey.
  initHeroConvergenceAnimation();
  initHeroWorldTransition();
  initSectionContinuity();
  initIkigaiJourney();
...
/* ==========================================================================
   1c. SECTION CONTINUITY — THE OVERLAP SYSTEM
   Generalizes the hero → philosophy handoff above (initHeroWorldTransition)
   across every remaining section boundary, so entering ANY section feels
   like a continuation of the same move rather than a fresh chapter
   starting cold. Two scrubbed passes per section, never a fixed-duration
   tween:
     - ARRIVAL: the section fades/lifts/sharpens into place as it enters,
       resolved by the time it's ~45% up the viewport — the same curve
       initHeroWorldTransition already uses for "why-ikigai", so the two
       systems read as one language rather than two different effects.
     - RECESSION: a light, deliberately subtle softening as the section
       leaves — never a full fade, just enough that the next section is
       visibly already present in the same frame before the previous one
       is gone. This is the "next section begins before the previous one
       has ended" overlap the whole pass is built around.
   Each section's header block gets a second, tighter pass of the same
   language so the outgoing heading and the incoming one briefly coexist
   — old + new on screen together for a beat, per the typography note.
   Hero and "why-ikigai" are excluded here; that handoff already has its
   own richer treatment above, and wiring the generic version onto them
   too would just double up the same ScrollTriggers.
   ========================================================================== */
function initSectionContinuity() {
  if (motion.reducedMotion) return;
  const gsapLib = window.gsap;
  const ScrollTriggerLib = window.ScrollTrigger;
  if (!gsapLib || !ScrollTriggerLib) return;

  const sectionIds = ["contrast", "identities", "journey", "organizers", "partners", "faq", "climax"];
  const sections = sectionIds.map((id) => document.getElementById(id)).filter(Boolean);

  sections.forEach((section) => {
    gsapLib.fromTo(
      section,
      { scale: 0.96, opacity: 0.6, y: 40, filter: "blur(4px)" },
      {
        scale: 1, opacity: 1, y: 0, filter: "blur(0px)", ease: "none",
        scrollTrigger: { trigger: section, start: "top bottom", end: "top 45%", scrub: true },
      }
    );
    ...
    gsapLib.fromTo(
      ...
      {
        scale: 0.985, opacity: 0.82, filter: "blur(1.5px)", ease: "none",
        scrollTrigger: { trigger: section, start: "bottom 55%", end: "bottom 4%", scrub: true },
      }
    );

    const header =
      section.querySelector(".section-header-editorial") ||
      section.querySelector(".forest-inner-layout") ||
      section.querySelector(".climax-heading");
    if (!header) return;

    gsapLib.fromTo(
      header,
      { y: 26, opacity: 0.4, filter: "blur(3px)" },
      {
        y: 0, opacity: 1, filter: "blur(0px)", ease: "none",
        scrollTrigger: { trigger: header, start: "top bottom", end: "top 60%", scrub: true },
      }
    );
  });
}
...
/* ==========================================================================
   5. THE ORGANIZERS (CORE TEAM GRID)
   ========================================================================== */
function initOrganizers() {
  const grid = document.getElementById("organizersGrid");
  if (!grid || !organizers) return;

  grid.innerHTML = organizers.map(person => `
    <div class="organizer-card ${person.isAnnounced ? "" : "organizer-card--open"}" data-id="${person.id}">
      <div class="organizer-photo-frame">
        ${
          person.isAnnounced && person.photo
            ? `<img class="organizer-photo" src="${person.photo}" alt="${person.name}, ${person.role}" loading="lazy" />`
            : `<div class="organizer-photo-placeholder" aria-hidden="true">
                 <span>?</span>
               </div>`
        }
        <span class="organizer-card-id">${person.id}</span>
      </div>
      <div class="organizer-info">
        <h3 class="organizer-name ${person.isAnnounced ? "organizer-name--signature" : ""}">${person.name}</h3>
        <p class="organizer-role">${person.role}</p>
      </div>
    </div>
  `).join("");
}
...
function initSponsorsAndAlliance() {
  ...
  // Render Community Alliance (German Crab gets the featured showcase
  // treatment; everything else stays a compact standard card).
  if (allianceGrid && communityAlliance) {
    allianceGrid.innerHTML = communityAlliance.map(partner => `
      <div class="alliance-card ${partner.featured ? "alliance-card--featured" : ""}">
        ${
          partner.featured
            ? `<div class="alliance-featured-icon" aria-hidden="true">${
                partner.icon && /\.(png|jpe?g|svg|webp)$/i.test(partner.icon)
                  ? `<img class="alliance-featured-icon__img" src="${partner.icon}" alt="" loading="lazy" />`
                  : (partner.icon || "★")
              }</div>`
            : ""
        }
        <div class="alliance-card-body">
          <div class="alliance-partner-type">${partner.type || "COMMUNITY PARTNER"}</div>
          <div class="alliance-partner-name">${partner.name}</div>
          <div class="alliance-partner-role">${partner.role || ""}</div>
          ${
            partner.featured && partner.code
              ? `<div class="alliance-featured-code">${partner.code}</div>`
              : ""
          }
        </div>
      </div>
    ...
    <div class="faq-accordion-item ${idx === 0 ? 'open' : ''}">
      <button type="button" class="faq-trigger" aria-expanded="${idx === 0 ? 'true' : 'false'}">
        <span class="faq-trigger-text">
          <span class="faq-trigger-meta">${faq.index} // ${faq.tag}</span>
    ...
    function runCommand(cmd) {
      ...
      switch (cmd) {
        ...
        case "journey":
          ...
          break;
        case "partners":
          append("Partnership at Ikigai is an investment in human capability. Inquiries: ikigai.delhi.kalicoders@gmail.com");
          break;
        case "clear":
          logs.innerHTML = "";
          break;
...
function initTextFadeInAnimation() {
  ...
  candidateElements.forEach((el, index) => {
    ...
    if (parent && (parent.classList.contains("identities-grid") ||
                   ...
                   parent.classList.contains("organizers-grid"))) {
...
function initCustomCursor() {
  ...
  const HOVER_SELECTOR = "a, button, .organizer-card, .identity-card, .faq-trigger, .hero-convergence-stage";
...
function initSectionNumberMorphing() {
  ...
  const systemSections = [
    ...
    { id: "identities", num: "03", name: "SPECIFICATIONS" },
    { id: "journey", num: "04", name: "THE JOURNEY" },
    { id: "organizers", num: "05", name: "ORGANIZERS" },
    { id: "partners", num: "06", name: "PARTNERS" },
    { id: "faq", num: "07", name: "DIRECTORY" },
    { id: "climax", num: "08", name: "CONVERGENCE" }
  ];
...
function logEditorialConsoleBanner() {
  ...
  console.log(
    "%cInquiries: ikigai.delhi.kalicoders@gmail.com",
